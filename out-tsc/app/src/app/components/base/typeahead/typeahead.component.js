...
var WikipediaService = /** @class */ (function () {
    ...
    WikipediaService.prototype.search = function (term) {
        ...
        return this.http
            .get(WIKI_URL, { params: PARAMS.set('search', term) }).pipe(map(function (response) { return response[1]; }));
    };
...
var TypeaheadComponent = /** @class */ (function () {
    function TypeaheadComponent(_service) {
        ...
        this.searchOnFocus = function (text$) {
            ...
            return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(map(function (term) { return (term === '' ? states
                : states.filter(function (v) { return v.toLowerCase().indexOf(term.toLowerCase()) > -1; })).slice(0, 10); }));
        };
        this.searchWikipedia = function (text$) {
            return text$.pipe(debounceTime(300), distinctUntilChanged(), tap(function () { return _this.searching = true; }), switchMap(function (term) {
                return _this._service.search(term).pipe(tap(function () { return _this.searchFailed = false; }), catchError(function () {
                    _this.searchFailed = true;
                    return of([]);
                }));
            }), tap(function () { return _this.searching = false; }));
        };
        this.searchTemp = function (text$) {
            return text$.pipe(debounceTime(200), map(function (term) { return term === '' ? []
